...
(`${CLAUDE_PLUGIN_ROOT}/scripts/block-dangerous-commands.sh`). It intercepts
each shell command and asks for explicit confirmation before anything
destructive. This skill makes that posture **active and visible**: you confirm
what's watched, and you commit to the discipline below.

You do not need to install anything here — the hook is active because the
plugin's hooks are enabled. This skill is the behavioral contract that goes
with it.
