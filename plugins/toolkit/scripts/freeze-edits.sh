...
reason="FROZEN: edits are restricted to ${boundary}. '${abs}' is outside the boundary. To allow it, run: bash \"\${CLAUDE_PLUGIN_ROOT}/scripts/unfreeze-edits.sh\" (or edit .claude/session-state/freeze-boundary)."
if command -v jq >/dev/null 2>&1; then
  jq -nc --arg r "$reason" '{
    hookSpecificOutput: {
