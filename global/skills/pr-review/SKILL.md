...
- **Labels:** prefix with `nit:` (non-blocking cosmetic), `issue:` (a real
  problem), or `suggestion:` (an optional improvement) when it helps the author
  prioritize, and mark non-blocking things as such. **Do not label questions or
  praise.** Write them plainly, as a normal sentence.
